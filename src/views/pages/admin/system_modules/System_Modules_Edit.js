...
class System_Modules_Edit extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    /* 
      Get User deatils 
    */
    systemModulesService.getSystemModule(this.state.fields.id).then((res) => {
      if (res.status === false) {
        notify.error(res.message);
      } else {
        if (res.data == null) {
          notify.error("System Module not found");
          history.push("/admin/system_module");
        }
  ...
  handleSubmit() {
    if (this.validator.allValid()) {
      systemModulesService.updateSystemModule(this.state.fields).then((res) => {
        if (res.status === false) {
          notify.error(res.message);
        } else {
            ...
            <CCard>
              ...
              <CCardBody>
                ...
                <CFormGroup>
                    ...
                    validator={(value) => {
                      if (this.state.fields.action.includes(value)) {
                        ...
                        if (!/[^a-z]/.test(value)) {
                          ...
                          $(".error-action").html(
                            '<div class="text-danger">Uppercase and special character not allow</div>'
                          );
                        }
                      }
                    }}
                  />
                  <CFormText className="help-block error-action">
                    {this.validator.message(
                      "action",
                  ...
                  <CFormText className="help-block">
                    <b>Note : </b>This action field is used in ACL permission
                    module.
                  </CFormText>
                </CFormGroup>
              </CCardBody>
...
export default System_Modules_Edit;
