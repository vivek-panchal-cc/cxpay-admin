...
class SMS_Template_Edit extends Component {
  ...
  componentDidMount() {
    this.getMedia();
    setTimeout(() => {
      if (_canAccess(this.props.module_name, this.props.action, '/admin/sms_templates')) {
        const slug = slugify(this.state.name, { delimiter: '-' });
        var postData = { id: this.state.id }
        smsTemplateService.getpage(postData).then(res => {
  ...
  checkValidation(event) {
    if (this.validator.allValid()) {
      ...
      smsTemplateService.updatepage(postData).then(res => {
        if (res.status === 'error') {
          notify.error(res.message);
        }
        else {
      ...
      <CCard>
        ...
        <CCardBody>
          <CFormGroup>
            ...
            <CInput type="text" id="name" name="name" placeholder="Enter Name" autoComplete="name" onChange={this.handleChange} value={this.state.name} />
            <CFormText className="help-block">{this.validator.message('name', this.state.name, 'required', { className: 'text-danger' })}</CFormText>
          </CFormGroup>
...
export default SMS_Template_Edit