...
class SMS_Template_Add extends Component {
  ...
  componentDidMount() {
    this.getMedia();
    setTimeout(() => {
      _canAccess(this.props.module_name, this.props.action, '/admin/sms_templates')
    }, 300);
  }
  ...
  checkValidation(event) {
    if (this.validator.allValid()) {
      ...
      smsTemplateService.createPages({
      ...
      <CCard>
        ...
        <CCardBody>
          <CFormGroup>
            ...
            <CInput type="text" id="title" name="name" placeholder="Enter Title" autoComplete="title" onChange={this.handleChange} />
            <CFormText className="help-block">{this.validator.message('title', this.state.name, 'required', { className: 'text-danger' })}</CFormText>
          </CFormGroup>
          <CFormGroup>

            <CLabel htmlFor="nf-name">Slug</CLabel>
            <CInput type="text" name="slug" id="slug" placeholder="Enter Slug" value={slugify(this.state.name, { delimiter: '-' })} onChange={this.handleChange} />
          ...
          <CFormGroup>
            ...
            <Editor
              ...
              init={{
                ...
                toolbar: 'insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link  preview myCustomToolbarButton ',
...
export default SMS_Template_Add